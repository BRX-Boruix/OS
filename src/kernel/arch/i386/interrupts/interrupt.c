...
void interrupt_init(void) {
    ...
    idt_set_gate(IRQ1_KEYBOARD, (unsigned int)interrupt_handler_keyboard, 0x08, IDT_INTERRUPT_GATE);
    
    // 设置定时器中断 (IRQ0 -> INT 0x20)
    idt_set_gate(IRQ0_TIMER, (unsigned int)interrupt_handler_timer, 0x08, IDT_INTERRUPT_GATE);
